...
CGraphicRenderTargetTexture::CGraphicRenderTargetTexture()
	: m_d3dFormat{ D3DFMT_A8R8G8B8 }, m_depthStencilFormat{ D3DFMT_UNKNOWN }
{
	Initialize();
...
bool CGraphicRenderTargetTexture::Create(const int width, const int height, const D3DFORMAT texFormat, const D3DFORMAT depthFormat)
{
	...
	{
		TraceError("CGraphicRenderTargetTexture::Create cannont CreateRenderTexture");
		return false;
	}

	if (!CreateRenderDepthStencil(width, height, depthFormat))
	{
		TraceError("CGraphicRenderTargetTexture::Create cannont CreateRenderDepthStencil");
		return false;
	}
...
{
	m_d3dFormat = format;
	if (FAILED(ms_lpd3dDevice->CreateTexture(width, height, 0, D3DUSAGE_RENDERTARGET, m_d3dFormat, D3DPOOL_DEFAULT, &m_lpd3dRenderTexture, NULL)))
		return false;
